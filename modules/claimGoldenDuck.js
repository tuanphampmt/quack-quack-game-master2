  ...
  try {
    ...
    if (error.response) {
      ...
      if (status >= 500) {
        ...
        logger.error(`claimGoldenDuckInternal error ${status}`, "error");
        await sleep(5);
        return null;
      } else if (status === 401) {
        ...
        logger.error(
          `claimGoldenDuckInternal error ${status} undefined`,
          "error"
        );
        await sleep(3);
        return null;
      ...
      console.log("Lost connect, auto connect after 3s, retry to die");
      logger.error(
        `claimGoldenDuckInternal error request ${error.request}`,
        "error"
      );
      await sleep(3);
      return null;
      ...
      console.log("Lost connect, auto connect after 3s, retry to die");
      logger.error(`claimGoldenDuckInternal error ${error.message}`, "error");
      await sleep(3);
      return null;
    }
  }
